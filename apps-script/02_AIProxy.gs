...
function buildAiPromptInner_(type, payload) {
  ...
  if (type === 'summary') {
    ...
    return 'Experienced middle school AP reviewing a completed Camp Mountaineer Trail Journal. Uses CPS, restorative practices, logical consequences.\n\n' +
      ...
      tierNote + '\n\n' +
      'INSIGHT LEVEL: Calibrate to a real middle schooler, roughly age 11-14, writing under time pressure in a detention or office setting -- not an adult\'s emotional vocabulary or polished self-reflection. Brief, plainly-worded, imperfectly-spelled answers are the NORM at this age, not a sign of low effort or low insight. HIGH = honest effort throughout + named someone hurt + any self-awareness + non-dismissive, even if short or bluntly worded -- a one-sentence honest answer from a 12-year-old should score the same as a longer one that shows the same honesty. MEDIUM = partial/thin engagement, OR effort is present but the account exclusively blames others with zero ownership of their own role. LOW = clearly avoidant/dismissive throughout only -- e.g. "idk" to everything, refusing to engage, or answers that show no attempt at all. Default strongly toward HIGH: most genuine attempts at this age should qualify, and brevity or informal writing is never itself a reason to score lower.\n\n' +
